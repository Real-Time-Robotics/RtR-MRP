    ...
    const fetchPart = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const found = data.data?.find((p: any) => p.partNumber.toLowerCase() === partNumber.toLowerCase());
          if (found) {
            setPartInfo(found);
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <input type="text" value={partNumber} onChange={(e) => setPartNumber(e.target.value.toUpperCase())} placeholder="RTR-MOTOR-001" className="flex-1 px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-xl font-mono" />
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-4 space-y-3">
        ...
        {partInfo?.locations?.length > 0 ? (
          ...
          <input type="text" value={fromLocation} onChange={(e) => setFromLocation(e.target.value.toUpperCase())} className="w-full px-4 py-3 bg-gray-100 rounded-xl font-mono" />
        )}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-4 space-y-3">
        ...
        <input type="text" value={toLocation} onChange={(e) => setToLocation(e.target.value.toUpperCase())} placeholder="WH-01-R02-C01-S01" className="w-full px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-xl font-mono" />
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-4 space-y-3">
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <input type="number" value={quantity} onChange={(e) => setQuantity(Math.max(1, parseInt(e.target.value) || 1))} className="w-24 h-14 text-center text-2xl font-bold bg-gray-100 rounded-xl" />
      ...
      <button onClick={handleSubmit} disabled={!partInfo || !fromLocation || !toLocation || isSubmitting}
        ...
        {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : <><ArrowRight className="w-5 h-5" />Xác nhận chuyển</>}
      </button>
    </div>
  );
}
