...
const navItems: NavItem[] = [
  ...
  { id: 'orders', label: 'Đơn hàng', icon: <ShoppingCart className="w-5 h-5" />, href: '/supplier/orders', badge: 3 },
...
const mockSupplier = {
  id: 'SUP-001',
  name: 'ABC Components Co., Ltd',
  code: 'ABC-VN',
  rating: 'A',
  email: 'contact@abc-components.vn',
};
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/supplier" className="flex items-center gap-3">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-xs text-gray-500">RTR MRP</p>
              </div>
            </Link>
          </div>
            ...
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                  <span className="text-sm font-bold text-blue-600">{mockSupplier.code.slice(0, 2)}</span>
                </div>
                <div className="hidden md:block text-left">
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{mockSupplier.code}</p>
                  <p className="text-xs text-gray-500">Rating: {mockSupplier.rating}</p>
                </div>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
                  <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                    <p className="font-medium text-gray-900 dark:text-white">{mockSupplier.name}</p>
                    <p className="text-sm text-gray-500">{mockSupplier.email}</p>
                  </div>
                  <Link
