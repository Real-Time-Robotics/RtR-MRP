...
async function getInventoryMetrics(startDate: Date) {
  try {
    ...
    const totalValue = inventoryItems.reduce((sum, item) => {
      return sum + (Number(item.quantity) * Number(item.part?.unitCost || 0));
    }, 0);
...
async function getInventoryByCategory() {
  try {
    const inventoryItems = await prisma.inventory.findMany({
      include: {
        part: {
          select: { category: true, unitCost: true }
        }
      }
    });
...
async function getTopParts() {
  try {
    const inventoryItems = await prisma.inventory.findMany({
      include: {
        part: {
          select: { name: true, partNumber: true, unitCost: true }
        }
      }
    });
