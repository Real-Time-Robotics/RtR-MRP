// POST /api/production/daily-plan/:id/approve
...
export const POST = withAuth(async (_request: NextRequest, context: RouteContext, session) => {
  ...
  if (plan.lines.length === 0) {
    return NextResponse.json({ error: 'Cannot approve empty plan' }, { status: 422 });
  }
  if (plan.status !== 'DRAFT') {
    return NextResponse.json({ error: `Plan is ${plan.status}, not DRAFT` }, { status: 409 });
  }
